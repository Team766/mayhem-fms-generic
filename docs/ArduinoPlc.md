...
- **This needs a firmware change and new fixed addresses**, so extend `plc/mayhem_arduino_test.go` with them, and size the coil table in the firmware to match. It serves 32 coils today: 8 generic + 12 light coils for four stations + 1 flag = 21 fits, and so would lights at station 3 (27).
