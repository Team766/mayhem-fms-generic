...
func TestSingleGameUntilFinalsInitialWith4Alliances(t *testing.T) {
	finalMatchup, breakSpecs, err := newSingleGameUntilFinalsBracket(4)
	assert.Nil(t, err)
...
func TestSingleGameUntilFinalsInitialWith8Alliances(t *testing.T) {
	finalMatchup, breakSpecs, err := newSingleGameUntilFinalsBracket(8)
	assert.Nil(t, err)
	matchSpecs, err := collectMatchSpecs(finalMatchup)
	assert.Nil(t, err)
...
func TestSingleGameUntilFinalsErrors(t *testing.T) {
	_, _, err := newSingleGameUntilFinalsBracket(1)
	if assert.NotNil(t, err) {
		assert.Equal(t, "single-elimination bracket must have at least 2 alliances", err.Error())
	}

	_, _, err = newSingleGameUntilFinalsBracket(17)
	if assert.NotNil(t, err) {
...
func TestSingleGameUntilFinalsSemifinalProgression(t *testing.T) {
	playoffTournament, err := NewPlayoffTournament(model.SingleGameUntilFinalsPlayoff, 4)
	assert.Nil(t, err)
...
// Verifies that the final of the single-game-until-finals bracket is unaffected and still requires two wins, including the
// case of a 1-1 series that must be decided by a third match.
func TestSingleGameUntilFinalsFinalStillBestOfThree(t *testing.T) {
	playoffTournament, err := NewPlayoffTournament(model.SingleGameUntilFinalsPlayoff, 4)
	assert.Nil(t, err)
	finalMatchup := playoffTournament.FinalMatchup()
	playoffMatchResults := map[int]playoffMatchResult{
