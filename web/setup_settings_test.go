...
func TestSetupSettings(t *testing.T) {
	...
	recorder = web.postHttpResponse(
		...
		"name=Chezy Champs&code=CC&playoffType=single&numPlayoffAlliances=16&"+
			"eventCode=2014cc&teleopDurationSec=106&warningRemainingDurationSec=25&twoVsTwoMode=on",
	)
...
func TestSetupSettingsSingleEliminationSingleGame(t *testing.T) {
	web := setupTestWeb(t)

	recorder := web.postHttpResponse(
		"/setup/settings", "playoffType=SingleEliminationSingleGamePlayoff&numPlayoffAlliances=6",
	)
	assert.Equal(t, 303, recorder.Code)
	assert.Equal(t, model.SingleEliminationSingleGamePlayoff, web.arena.EventSettings.PlayoffType)
	...
	recorder = web.getHttpResponse("/setup/settings")
	assert.Equal(t, 200, recorder.Code)
	assert.Contains(t, recorder.Body.String(), "SingleEliminationSingleGamePlayoff")
	...
	recorder = web.postHttpResponse(
		"/setup/settings", "playoffType=SingleEliminationSingleGamePlayoff&numPlayoffAlliances=1",
	)
